...
void BOpenGLRenderer::init( int width, int height )
{
	...
	//HACK: Remove this. It's for texture testing purposes only
	/*
	GLuint tex = SOIL_load_OGL_texture("img_test.png",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
...
void BOpenGLRenderer::viewportChanged( int width, int height )
{
...
void BOpenGLRenderer::beforeRender()
{
	//setMaterial( mActiveMaterial);
	//TODO: The view matrix is sent in the wrong.
	mModelViewMatrix.PushMatrix( glm::value_ptr( mViewMatrix ) );
}

void BOpenGLRenderer::afterRender()
{
	mModelViewMatrix.PopMatrix();
}

void BOpenGLRenderer::loadMesh( RIDType mid, const BMesh &mesh )
{
...
void BOpenGLRenderer::renderMesh( BMesh::RID mid, const BNode3D &node )
{
	...
	//TODO:[Performance] Stop using matrix stacks. Or at least use GLM matrices
...
void BOpenGLRenderer::initShaders()
{
