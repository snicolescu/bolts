...
namespace LogMan
{
    ...
    void Init(TEnabler& enabler, const char* pFile, int line)
    {
        ...
        if (i != GetMap().end())
        {
            ...
            FILE_LOG(logDEBUG) << "Initialized the LogMan: '" << 
        ...
        else
        {
            ...
            FILE_LOG(logDEBUG) << "Initialized the LogMan for '" << 
    ...
    void Set(const char* pFile, int line, TEnabler enabler)
    {
        ...
        {
            TValue& value = i->second;
            FILE_LOG(logDEBUG) << "Changing the value in LogMan: '" << pFile << ":" << 
            ...
            {
                Write(enabler, **j);
                FILE_LOG(logDEBUG) << "Set the LogMan (initialized value): '" << 
        ...
        else
        {
            ...
            FILE_LOG(logDEBUG) << "Saved the value in LogMan (add to the map): '" << pFile << ":" << 
