...
namespace Bolts {
	namespace Rendering {
		...
		struct GLSLShader {
			...
			bool IsValid() { return (m_shaderID != 0); }
		...
		class GLSLProgram:public SmallRefCountT{
			public:
				...
				bool operator== (const GLSLProgram& other) { return m_programID != other.m_programID;}
				bool operator!= (const GLSLProgram& other) { return !(*this == other); }
				...
				void					Enable();
				const attributeList_t&	GetUsedAttributes() { return m_attributes; }
				const uniformList_t&	GetUsedUniforms() { return m_uniforms; }
